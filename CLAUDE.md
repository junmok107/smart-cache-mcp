...
- **캐시 용량**: 256MB / 50,000건 (초과 시 교체 정책 실행)
...
- **부팅 확인 (1단계 완료)**: `docker-compose up --build` 후 3개 컨테이너 모두 `healthy` 상태 확인됨
  - `mcp-server`: `GET http://localhost:3000/health` → `{"status":"ok"}` (Node 내장 `http` 모듈, MCP 프레임워크는 3단계에서 도입)
  - `embedding-service`: `GET http://localhost:8000/health` → `{"status":"ok"}` (FastAPI 스텁, 실제 모델 로딩은 2단계에서 구현)
...
| `CACHE_MAX_ENTRIES` | 최대 캐시 항목 수 (기본 50000) |
| `CACHE_MAX_SIZE_MB` | 최대 캐시 용량 MB (기본 256) |
