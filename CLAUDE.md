...
> **상태: 7단계 (벤치마크 및 토큰 절감 검증) 완료 — 전체 개발 단계 완료. 이후 사후 개선(보안·CI·재랭킹) 진행 중.** 이 문서는 개발이 진행되면서 실제 구현 내용에 맞게 계속 업데이트됩니다.
...
| 임베딩 서비스 | Python + FastAPI + `multilingual-e5-base` (768차원, ~1.1GB, HNSW 코사인 유사도 1단계) + `BAAI/bge-reranker-v2-m3` cross-encoder (~2.3GB, 재랭킹 2단계 — 11장 참고) |
...
- **코사인 유사도 임계값**: `0.90` (e5 모델 유사도가 0.7~1.0에 밀집되는 특성상 0.85는 오탐 위험이 있어 보수적으로 채택. `cache_config`로 런타임 조정 가능). **재랭킹 도입(11장) 이후로는 평상시 히트/미스를 결정하지 않고, 재랭킹 서비스 장애 시 폴백 판정에만 쓰인다** — 평상시 결정은 `CACHE_RERANK_THRESHOLD`(`0.60`)가 담당
- **재랭킹 점수 임계값**: `0.60` — 11장 "재랭킹" 참고. 실측으로 도출한 값(무관한 인자값 오탐 ~0.50, 진짜 매치 ~0.72~0.99 사이에 위치)
...
| `RERANK_MODEL_NAME` | 재랭킹 cross-encoder 모델 (`BAAI/bge-reranker-v2-m3`) — 11장 참고 |
| `EMBEDDING_DEVICE` | `cuda` 또는 `cpu` (RTX 3060 사용 시 `cuda`, 임베딩·재랭킹 두 모델 모두 이 설정을 공유) |
| `CACHE_SIMILARITY_THRESHOLD` | 1단계 코사인 유사도 임계값 (기본 0.90). 재랭킹 서비스 장애 시 폴백 판정에만 사용 |
| `CACHE_RERANK_THRESHOLD` | 2단계 재랭킹 점수 임계값 (기본 0.60) — 평상시 히트/미스를 실제로 결정 |
| `CACHE_RERANK_TOP_K` | 재랭킹에 넘길 1단계 후보 개수 (기본 5) |
...
## 11. 재랭킹 (retrieve-then-rerank, 사후 개선, 2026-07-14)

기획서에는 없는 신규 기능. 7단계 벤치마크와 라이브 검증 중 문서화해둔 두 가지 오탐/누락 사례 — ① 교차언어 매칭 누락(`"What is artificial intelligence?"`가 `"인공지능이란 무엇인가요?"`와 매칭 안 됨, `docs/final-status.md` "정직하게 밝히는 한계")과 ② 구조적으로 유사한 JSON 인자 오탐(`roll_dice {"sides":6}` vs `{"sides":100}`, CLAUDE.md 4단계 "퍼지 매칭 관련 발견") — 이 둘 다 "벡터 하나로 코사인 유사도만 보고 즉시 히트/미스를 결정"하는 단일 단계 구조의 근본 한계라고 판단해, 검색-후-재랭킹(retrieve-then-rerank) 2단계 구조로 전환하기로 사용자와 논의 후 결정함.

### 아키텍처

```
1단계 (retrieve): embedQuery → pgvector HNSW 코사인 top-K(기본 5, CACHE_RERANK_TOP_K) 후보 조회, 임계값 미적용
2단계 (rerank):   cross-encoder가 (query, candidate) 쌍을 함께 인코딩해 각 후보를 채점
                   최고 점수 후보가 CACHE_RERANK_THRESHOLD(기본 0.60) 이상이면 히트
```

1단계만으로는 "쿼리 벡터"와 "후보 벡터"를 독립적으로 인코딩한 뒤 코사인 거리만 재는 구조라 두 텍스트를 직접 비교하지 못하지만, cross-encoder는 두 텍스트를 하나의 입력으로 함께 인코딩해 토큰 단위 상호작용을 모델이 직접 볼 수 있어 훨씬 더 정교한 판별이 가능하다 — 대신 후보 하나당 추론을 한 번씩 해야 해서 1단계보다 비싸므로, 전체 테이블이 아니라 1단계가 좁혀준 top-K에만 적용한다.

### 재랭킹 모델 선택 — 실측으로 뒤집힌 첫 번째 선택

처음에는 가볍고 빠른 `cross-encoder/mmarco-mMiniLMv2-L12-H384-v1`(다국어 MS MARCO 학습, MiniLM 기반 118M 파라미터)를 기본값으로 채택했으나, 실제 `/rerank` 엔드포인트를 붙여 라이브로 점수를 찍어보니 **완전히 반대 방향으로 실패**하는 것을 발견함:

| 케이스 | 점수 | 기대 |
|---|---|---|
| `get_weather{city:Seoul}` vs `get_weather{city:Busan}` (다른 도시, 미스여야 함) | **0.968** | 낮아야 함 |
| `roll_dice{sides:6}` vs `{sides:100}` (다른 값, 미스여야 함) | **0.83** | 낮아야 함 |
| `서울 날씨 어때?` vs `서울의 오늘 날씨를 알려줘` (같은 언어 패러프레이즈, 히트여야 함) | 0.026 | 높아야 함 |

원인은 이 모델이 MS MARCO 패시지 랭킹(= "이 문서가 이 쿼리와 주제적으로 관련 있는가")으로 학습되어 있어서, **"같은 도구·같은 스키마"라는 주제적 유사성에는 강하게 반응하지만 인자값 하나가 바뀌어 의미가 달라졌다는 사실은 거의 무시**한다는 것 — 정확히 우리가 재랭킹으로 고치려던 오탐(구조적으로 유사한 JSON 인자)을 오히려 악화시키는 방향이었다. "주제 관련성"과 "의미적 동등성"은 다른 과제이고, 검색용 relevance 랭커는 후자에 안 맞는다는 걸 실측으로 확인함.

대안으로 더 크고 최신인 `BAAI/bge-reranker-v2-m3`(568M 파라미터, 다국어)를 같은 케이스로 실측:

| 케이스 | 점수 |
|---|---|
| 완전 동일 텍스트 | 0.9998 |
| `get_weather{city:Seoul}` vs `get_weather{city:Seoul}` (동일) | 0.731 |
| `get_weather{city:Seoul}` vs `{city:Busan}` (다른 도시) | **0.502** |
| `roll_dice{sides:6}` vs `{sides:100}` (다른 값) | **0.505** |
| `What is artificial intelligence?` vs `인공지능이란 무엇인가요?` (교차언어 패러프레이즈) | **0.731** |
| `서울 날씨 어때?` vs `서울의 오늘 날씨를 알려줘` (동일언어 패러프레이즈) | 0.724 |

진짜 매치(0.72~0.99)와 인자값 오탐(0.50~0.51) 사이에 뚜렷한 간격이 생겨 `BAAI/bge-reranker-v2-m3`로 최종 채택함. VRAM ~2.3GB(e5-base ~1.3GB와 합쳐도 RTX 3060 8GB에 여유), top-5 후보 배치 재랭킹 지연시간 ~26ms(GPU) — 이 프로젝트가 이미 문서화해둔 "캐시 hit가 목업 기준선보다 느리다"는 한계(`docs/final-status.md`)를 재랭킹이 한 단계 더 키우는 셈이지만, 실제로 무거운 원본 MCP 앞에서는 무시할 수 있는 수준일 것으로 판단해 감수하기로 함(검증은 안 함, 기존 한계와 동일한 성격).

### 아직 안 풀리는 한계 — ML vs DL

교차언어·인자값 오탐은 개선됐지만, "밀접하게 관련된 서로 다른 개념"은 여전히 못 가른다는 것을 라이브 검증으로 확인함:

| 케이스 | 점수 |
|---|---|
| `머신러닝이 뭔가요?` vs `딥러닝이 뭔가요?` (같은 템플릿 문장) | **0.719** |
| `머신러닝이 뭔가요?` vs `딥러닝에 대해 설명해주세요` (다르게 표현) | **0.603** |
| (대조) `What is AI?` vs `인공지능이란?` (진짜 매치) | 0.731 |

딥러닝이 머신러닝의 하위 개념이라 실제로 의미상 가깝기는 하지만, 서로 다른 질문에 대해 다른 답(ML 설명 vs DL 설명)을 내놔야 하므로 오탐이다. 문제는 이 점수(0.60~0.72)가 진짜 매치의 점수대(0.72~0.99)와 거의 겹쳐서, **임계값을 어떻게 조정해도 둘을 깔끔하게 가를 수 없다** — 임계값을 0.73 이상으로 올리면 이 오탐은 막지만 교차언어 진짜 매치(0.731)까지 같이 걸러져 버린다. `CACHE_RERANK_THRESHOLD=0.60`을 유지하기로 한 이유는 인자값 오탐·교차언어 누락이라는, 실제로 더 흔하고 명확한 두 문제를 해결하는 게 이득이 크고, ML/DL류의 "밀접히 관련된 개념 간 구분"은 애초에 재랭킹 도입 전부터(4단계 기록) 알려진 미해결 리스크였기 때문 — 재랭킹은 이 리스크를 완전히 없애지 못했지만 악화시키지도 않았고(단일 코사인 단계였을 때도 동일한 종류의 오탐 위험이 문서화되어 있었음), 다른 두 축은 실측으로 개선을 확인함.

### 구현

- `embedding-service/app.py`: 기동 시 e5-base와 `RERANK_MODEL_NAME`(cross-encoder)를 함께 로드. `POST /rerank` — `{query, candidates}` → `{scores}` (sigmoid 적용해 [0,1]로 반환; 원본 모델 출력은 unbounded logit). 쿼리·후보 텍스트 모두 e5의 `query:`/`passage:` 프리픽스 없이 원문 그대로 전달 — 그 프리픽스는 e5 bi-encoder 전용 컨벤션이라 cross-encoder에는 무관함
- `embedding-service/prefetch_model.py` / `Dockerfile`: 재랭킹 모델도 빌드 시 함께 prefetch (런타임 인터넷 불필요 원칙 유지)
- `mcp-server/src/db/cache-entries.ts`: `findBySimilarity`(top-1) → `findTopKBySimilarity`(top-K, 임계값 미적용)로 교체
- `mcp-server/src/embedding/client.ts`: `rerank(query, candidates)` 추가
- `mcp-server/src/cache/cached-call.ts`의 `lookupFresh()`: embedQuery → top-K 조회 → 후보 텍스트를 `buildCacheText(tool_name, arguments_raw)`로 재구성(재저장 시 임베딩된 텍스트와 동일 — `buildCacheText`가 키 순서에 안 민감해서 JSONB 왕복 후에도 안전) → `rerank()` 호출 → 최고 점수가 임계값 이상이면 히트
  - **재랭킹 서비스 장애 시 우아한 성능 저하**: `rerank()`가 `EmbeddingServiceError`를 던지면(1단계 벡터 검색 자체는 성공한 상태) 1단계에서 이미 받아온 top-1 후보를 예전 방식대로 `CACHE_SIMILARITY_THRESHOLD`(코사인)로 판정 — 벡터 검색 결과를 버리지 않고 재랭킹 이전 동작으로 자연스럽게 폴백. 임베딩 서비스 자체가 완전히 죽은 경우(임베딩 단계 실패)는 기존과 동일하게 SHA-256 해시 폴백으로 감
  - `CachedCallResult.similarity` 필드가 이제 코사인이 아니라 재랭킹 점수를 담는다(단, 해시 폴백 히트는 여전히 `1`, 재랭킹 장애 시 폴백 히트는 코사인 원값) — 스케일이 경로별로 다르다는 걸 인지하고 사용해야 함
- `mcp-server/src/tools/cache-config.ts`: `rerank_threshold` 파라미터 추가 (기존 `similarity_threshold`와 동일하게 `process.env` 직접 기록 방식이라 재시작 불필요)

### 검증

- 단위/통합 테스트: `embedding-service` pytest에 `/rerank` 3개 추가(형태/빈 배열/관련성 우위) → 7개 전부 통과. `mcp-server` 통합 테스트에 "구조적으로 유사하지만 의미가 다른 호출은 재랭킹 후 미스여야 한다"는 회귀 테스트(`roll_dice sides:6 vs 100`) 추가, 기존 "동일 호출 히트" 테스트의 유사도 단언값도 코사인(0.9) 기준에서 재랭킹 임계값(0.6) 기준으로 갱신 → mcp-server 전체 14개 통합 테스트 통과
- 실제 라이브 검증(임시로 `benchmark/downstream-server.ts`의 지식조회 서버를 기동해 실제 `cached_call`로 확인, 확인 후 정리):
  - `인공지능이란 무엇인가요?` 캐싱 후 `What is artificial intelligence?` 호출 → `cache_hit: true, similarity: 0.731` (재랭킹 도입 전에는 미스였던 사례, 개선 확인)
  - `머신러닝이 뭔가요?` 캐싱 후 `딥러닝이 뭔가요?` 호출 → `cache_hit: true, similarity: 0.725` (위 "아직 안 풀리는 한계" 그대로 재현 — 오탐이지만 사전에 파악하고 문서화한 한계이지 새로 발견된 버그가 아님)
